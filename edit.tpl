...
<div class="card margin-bottom-no">
							<div class="card-main">
								<div class="card-inner">
									<div class="card-inner">
										<div class="cardbtn-edit">
												<div class="card-heading">弹窗显示设置</div>
												<button class="btn btn-flat" id="display-update"><span class="icon">check</span>&nbsp;</button>
										</div>
										<p class="card-heading"></p>
										<p>当前设置：<code id="ajax-display" data-default="display">{if $user->displayan==1}显示{else}不显示{/if}</code></p>
										<div class="form-group form-group-label control-highlight-custom dropdown">
											<label class="floating-label" for="display">显示设置</label>
											<button type="button" id="display" class="form-control maxwidth-edit" data-toggle="dropdown" value="{$user->displayan}">
												
											</button>
											<ul class="dropdown-menu" aria-labelledby="display">
												<li><a href="#" class="dropdown-option" onclick="return false;" val="1" data="display">显示</a> </li>
												<li><a href="#" class="dropdown-option" onclick="return false;" val="0" data="display">不显示</a></li>
											</ul>
										</div>
									</div>
								</div>
        ...
        $("#display-update").click(function () {
            $.ajax({
                ...
                success: function (data) {
                    if (data.ret) {
                        $("#result").modal();
						$("#ajax-display").html($("#display").val()=="1"?"显示":"不显示");
						$("#msg").html(data.msg);
                    } else {
                        $("#result").modal();
						$("#msg").html(data.msg);
